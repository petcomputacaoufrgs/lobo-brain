...
vector<vector<int>> Agent::choose_action(){
    ...
    if(random_prob() < this->epsilon){
        ...
        do{
            ...
            player_pos_idx = random_int(0, player_positions.size() - 1);
            
            cout << player_positions.at(0).at(0) << ";" << player_positions.at(0).at(1) << endl;
        ...
        action_idx = random_int(0, positions.size() - 1);
...
void Agent::feed_reward(float reward){
    for(vector<string>::iterator it = this->states.end();it != this->states.begin(); it--){
        ...
        // calculating delta
        float delta = this->alpha * (reward + (this->gamma * best_next_q_value) - this->Q[(*it)]);
